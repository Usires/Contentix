...
/**
 * Polyfill: crypto.randomUUID() is only available in secure contexts
 * (HTTPS or localhost). Contentix runs as a plain-HTTP LAN app
 * (http://asbach-games.fritz.box:PORT), so window.crypto.randomUUID is
 * undefined and createScript/createVideo throw "is not a function".
 *
 * This wrapper falls back to a v4-ish UUID built from getRandomValues,
 * which IS available everywhere (including insecure contexts).
 *
 * Used by store.js (createScript, createVideo). Symptom of the bug this
 * fixes: "crypto.randomUUID is not a function" when saving a new script/video.
 */
function uuidv4() {
  if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
    return crypto.randomUUID();
  }
  // Fallback: 16 random bytes → RFC 4122 v4 UUID string.
